...
async fn create_tmp_path(
    ...
    let lg_fpath = format!("{}/{}/1420-{}", &state.file_storage_path.tmp, &user_id, &filename);
    let md_fpath = format!("{}/{}/720-{}", &state.file_storage_path.tmp, &user_id, &filename);
    let sm_fpath = format!("{}/{}/340-{}", &state.file_storage_path.tmp, &user_id, &filename);
...
pub async fn upload_file(
    ...
    if let Some(metadata_field) = multipart.next_field().await? {
        ...
        if let Some(img_field) = multipart.next_field().await? {
            ...
            let tmp_upload_path = &format!("{}/{}", &state.file_storage_path.tmp, &user_id);
...
pub async fn get_blog_file(
    ...
    let file_path = format!("{}/{}/{}-{}", &state.file_storage_path.blog, uid, size, filename);
...
pub async fn get_book_file(
    ...
    let file_path = format!("{}/{}/{}-{}", &state.file_storage_path.book, uid, size, filename);
...
pub async fn get_tmp_file(
    ...
    let file_path = format!("{}/{}/{}-{}", &state.file_storage_path.tmp, uid, size, filename);
    // Attempt to read the file contents
    let f = std::fs::read(&file_path)?;
